...
class InventoryTest : FunSpec({
    val apple = Product("1", "Apple", 0.5, "Fruit")
    var banana = Product("2", "Banana", 0.3, "Fruit")
    var inventory: Inventory = Inventory()

    test("addProduct should increase product count and set producer") {
    ...
    test("removeProduct should decrease product count and remove producer") {
        inventory.addProduct(apple, "FruitCorp")
    ...
    test("addSeller should add seller to the inventory") {
        inventory.addSeller("SellerA")
        assertTrue(inventory.produceInventoryDisplay().contains("SellerA"))
    }

    test("removeSeller should remove seller from the inventory") {
        inventory.addSeller("SellerA")
    ...
    test("produceInventoryDisplay should display products and sellers correctly") {
        inventory.addProduct(apple, "FruitCorp")
    ...
    test("hasProducts should return true when there are products") {
        inventory.addProduct(apple, "FruitCorp")
        assertTrue(inventory.hasProducts())
    }

    test("hasProducts should return false when there are no products") {
        assertFalse(inventory.hasProducts())
    }

    test("getProducer should return null for nonexistent product") {
        assertNull(inventory.getProducer(apple))
    }
})